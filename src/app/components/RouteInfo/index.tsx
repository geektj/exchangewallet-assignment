...
export const RouteInfo: FC = () => {
  const { showRoute, setShowRoute, selectedRoute } = useExchange();
  const showRoutes = () => {
  ...
  if (!selectedRoute) {
    return (
      <div className="py-3 px-4 border-[2px] rounded-[12px] flex gap-1 sm:gap-0 sm:justify-between mx-auto cursor-pointer border-border1 bg-white shadow-custom-light">
        <div>No routes is selected</div>
        <div className="flex items-center gap-1" onClick={showRoutes}>
        <p className="text-purple font-manrope font-600 text-sm">
          View all routes
        </p>
    ...
    <div className="py-3 px-4 border-[2px] rounded-[12px] flex gap-1 sm:gap-0 sm:justify-between mx-auto cursor-pointer border-border1 bg-white shadow-custom-light">
      <div className="flex w-full md:w-3/4 xl:w-[70%] flex-wrap gap-[3px] md:gap-2 items-center">
        ...
        <div className="flex items-center gap-1">
          <p className="font-manrope font-600 text-sm text-darkText ">
            {selectedRoute?.routeName}
          </p>
          <span className="font-manrope text-darkText text-sm font-400">→</span>
        </div>
        <div className="flex gap-1 items-center h-[24px] rounded-[6px] bg-purpleLight py-[4px] px-[6px] border-[1px]">
          ...
          <p className="text-purple1 font-600 font-manrope text-xxxs">
            {selectedRoute?.price}
          </p>
        </div>
        ...
        {selectedRoute?.isReturn ? (
          ...
          <div className="bg-dangerAccent h-[24px] rounded-[6px] px-[6px] py-[4px] flex items-center justify-center font-manrope font-600 text-xxxs text-textDanger">{selectedRoute?.returnPercentage}</div>
        )}
